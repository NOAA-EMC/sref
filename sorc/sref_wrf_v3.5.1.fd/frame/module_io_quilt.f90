...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ...
          CALL collect_on_comm_debug("module_io_quilt.F",702, mpi_comm_io_groups(1), &
                                onebyte, &
                                dummy, 0, &
                                obuf, obufsize )
        ELSE
        ...
        DO WHILE ( retval )
          IF ( mytask_local .EQ. ntasks_local_group-1 ) THEN
            ...
            ALLOCATE( bigbuf(1) )
          ENDIF
          CALL collect_on_comm_debug2("module_io_quilt.F",932,Trim(VarName), &
...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ALLOCATE( obuf( (obufsize+1)/itypesize ) )
          CALL collect_on_comm_debug("module_io_quilt.F",1698, mpi_comm_io_groups(1), &
                                onebyte, &
                                dummy, 0, &
                                obuf, obufsize )
        ELSE
...
SUBROUTINE wrf_quilt_open_for_write_begin( FileName , Comm_compute, Comm_io, SysDepInfo, &
  ...
  CALL collect_on_comm_debug("module_io_quilt.F",2982, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_open_for_write_commit( DataHandle , Status )
  ...
  CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
  CALL collect_on_comm_debug("module_io_quilt.F",3058, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_iosync ( DataHandle, Status )
  ...
  IF ( associated ( int_local_output_buffer ) ) THEN
    ...
    IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
    CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
    CALL collect_on_comm_debug("module_io_quilt.F",3225, comm_io_group, &
                          onebyte, &
...
SUBROUTINE wrf_quilt_set_time ( DataHandle, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",3498, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_real ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",3616, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_integer ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",3776, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_char ( DataHandle, Element, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",3947, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_var_ti_char ( DataHandle,Element, Varname, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",4479, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
"frame/module_io_quilt.F: wrf_quilt_write_field: DataHandle not opened" )
  ...
  IF ( .NOT. okay_to_write( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      CALL MPI_Reduce( reduced, reduced_dummy, 2, MPI_INTEGER, MPI_SUM, tasks_in_group-1, comm_io_group, ierr )
      CALL collect_on_comm_debug("module_io_quilt.F",4901, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
  ELSE
