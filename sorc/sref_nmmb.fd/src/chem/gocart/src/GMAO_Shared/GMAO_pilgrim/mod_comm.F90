...
! !PUBLIC MEMBER FUNCTIONS:
      public mp_init, mp_exit,                                             &
             ...
             mp_swapirr, mp_swapirr_r4, mp_swapirr_i4, mp_barrier,         &
...
!
        if ( send_bl(ipe)%type /= MPI_DATATYPE_NULL ) then
          ...
          if (hs_local) then
             if (ipe-1 /= comm_pid) &
               call MPI_RECV ( signal, 1, mp_i4, ipe-1, comm_pid, comm, Status, ierr )
             if (send_local) then
...
! nonblocking send
            ...
            if (hs_local) then
               if (r8_win%dest /= comm_pid) &
                 call MPI_RECV (signal, 1, mp_i4, r8_win%dest, send_tag, comm, Status, ierr)
               if (send_local) then
...
! nonblocking send
            ...
            if (hs_local) then
               if (i4_win%dest /= comm_pid) &
                 call MPI_RECV (signal, 1, mp_i4, i4_win%dest, send_tag, comm, Status, ierr)
               if (send_local) then
...
!------------------------------------------------------------------------------
!BOP
! !ROUTINE: mp_swapirr_r4 --- Write r4 contiguous parcels to global array
!                             using XOR swap ordering
!
! !INTERFACE:
      subroutine mp_swapirr_r4 ( comm, send_bl, recv_bl, a1in, a1out, &
                                 a2in, a2out, sw_handshake, sw_maxreq, &
                                 sw_alltoall, sw_send )
...
! !INPUT PARAMETERS:
      ...
      type(blockdescriptor), intent(in)  :: send_bl(:) ! send blocks
      type(blockdescriptor), intent(in)  :: recv_bl(:) ! receive blocks
      real(r4), intent(in) :: a1in(*)                  ! local data segment
      real(r4), optional, intent(in) :: a2in(*)        ! local data segment
      logical, optional, intent(in) :: sw_handshake    ! use flow control and 
                                                       !  ready send
      integer, optional, intent(in) :: sw_maxreq       ! maximum number of outstanding
                                                       !  MPI requests
...
! !OUTPUT PARAMETERS:
      real(r4), intent(out) :: a1out(*)                ! local output segment
      real(r4), optional, intent(out) :: a2out(*)      ! local output segment
!
! !DESCRIPTION:
!     XOR-ordered version of all-to-all communication
!
! WARNING: mod_comm parameter max_irr might need to be set larger than expected
!          when swapping two variables; specifically, max_irr must be at least
!          as large as the incoming r4_win%ncall_s + the number of variables to
!          be swapped
!
! !REVISION HISTORY: 
!    08.06.30   Worley      original: derived from mp_sendirr, but using 
...
!                            added flow control/ready send option and maxreq
!                            throttling, added alltoall option
!    09.04.01   Sawyer      R4 version derived from R8 version mp_swapirr
!
! !BUGS:
!
!EOP
...
!     num_r = 0 if this process is not part of the receiving decomposition
      ...
      onetwo = 1
      if (present(a2in)) onetwo = 2
      unitsize = r4_win%size/max_irr

! advance to unused portion of storage window
      r4_win%ncall_s = r4_win%ncall_s + 1

      if (r4_win%ncall_s .gt. max_irr-onetwo+1) then
         write(iulog,*) "mp_swapirr: insufficient window storage - exiting"
         write(iulog,*) "r4_win%ncall_s max_irr = ", r4_win%ncall_s, max_irr
         call exit(1)
      endif

! calculate send lengths and displacements
      offset_0 = (r4_win%ncall_s-1)*unitsize
      offset_s = offset_0
      sndlths(:) = 0
      sdispls(:) = 0
      arr_sndlths(:) = 0
...
! Post receive requests
         do istep=1,maxreq
            ...
            offset_r = rdispls(p)+1
            if (rcvlths(p) > 0) then
               call mpi_irecv ( ga_r4_r(offset_r), rcvlths(p), mp_r4, &
                                p, p, comm, rcvids(istep), ierr )
               if (handshake) then
                  call mpi_send( hs, 1, mp_i4, p, p, comm, &
                                 ierr )
...
! gather data into global send buffer
      do istep=1,steps
         ...
         if (sndlths(p) .ne. 0) then
            ...
            do j = 1, send_bl(p+1)%nparcels
               do i = 1, send_bl(p+1)%blocksizes(j)
                  ga_r4_s(offset_v+i) = a1in(send_bl(p+1)%displacements(j)+i)
               enddo
               offset_v = offset_v + send_bl(p+1)%blocksizes(j)
            enddo
            ...
            if ( present(a2in) ) then
               ...
               do j = 1, send_bl(p+1)%nparcels
                  do i = 1, send_bl(p+1)%blocksizes(j)
                     ga_r4_s(offset_v+i) = a2in(send_bl(p+1)%displacements(j)+i)
                  enddo
                  offset_v = offset_v + send_bl(p+1)%blocksizes(j)
               enddo
            endif
            ...
            if (sndlths(p) > 0) then
               if (handshake) then
                  call mpi_recv  ( hs, 1, mp_i4, p, comm_pid, comm, &
                                   OutStats, ierr )
                  if (sendd) then
                     call mpi_rsend( ga_r4_s(offset_s), sndlths(p), mp_r4, &
                                      p, comm_pid, comm, ierr )
                  else
                     call mpi_irsend( ga_r4_s(offset_s), sndlths(p), mp_r4, &
                                      p, comm_pid, comm, sndids(istep), ierr )
                  endif
               else
                  if (sendd) then
                     call mpi_send ( ga_r4_s(offset_s), sndlths(p), mp_r4, &
                                      p, comm_pid, comm, ierr )
                  else
                     call mpi_isend ( ga_r4_s(offset_s), sndlths(p), mp_r4, &
                                      p, comm_pid, comm, sndids(istep), ierr )
                  endif
               endif
            endif
...
! local copy to send buffer
      if (sndlths(comm_pid) .ne. 0) then
         ...
         do j = 1, send_bl(comm_pid+1)%nparcels
            do i = 1, send_bl(comm_pid+1)%blocksizes(j)
               ga_r4_s(offset_v+i) = a1in(send_bl(comm_pid+1)%displacements(j)+i)
            enddo
            offset_v = offset_v + send_bl(comm_pid+1)%blocksizes(j)
         enddo
         ...
         if ( present(a2in) ) then
            ...
            do j = 1, send_bl(comm_pid+1)%nparcels
               do i = 1, send_bl(comm_pid+1)%blocksizes(j)
                  ga_r4_s(offset_v+i) = a2in(send_bl(comm_pid+1)%displacements(j)+i)
               enddo
               offset_v = offset_v + send_bl(comm_pid+1)%blocksizes(j)
            enddo
         endif
         ...
         if (.not. alltoall) then
            ga_r4_r(rdispls(comm_pid)+1:rdispls(comm_pid)+rcvlths(comm_pid)) = &
               ga_r4_s(sdispls(comm_pid)+1:sdispls(comm_pid)+sndlths(comm_pid))
         endif

      endif

      if (alltoall) then
         call mpi_alltoallv (ga_r4_s, sndlths, sdispls, mp_r4, &
                             ga_r4_r, rcvlths, rdispls, mp_r4, &
                             comm, ierror)
      endif
...
! local copy from receive buffer
      if (rcvlths(comm_pid) .ne. 0) then
         ...
         do j = 1, recv_bl(comm_pid+1)%Nparcels
            do i = 1, recv_bl(comm_pid+1)%blocksizes(j)
               a1out(recv_bl(comm_pid+1)%displacements(j)+i) = ga_r4_r(offset_v+i)
            enddo
            offset_v = offset_v + recv_bl(comm_pid+1)%blocksizes(j)
         enddo
         ...
         if ( present(a2out) ) then
            ...
            do j = 1, recv_bl(comm_pid+1)%Nparcels
               do i = 1, recv_bl(comm_pid+1)%blocksizes(j)
                  a2out(recv_bl(comm_pid+1)%displacements(j)+i) = ga_r4_r(offset_v+i)
               enddo
               offset_v = offset_v + recv_bl(comm_pid+1)%blocksizes(j)
            enddo
         endif
...
! scatter data from global receive buffer to final destination
      do istep=1,steps
         ...
         if (rcvlths(p) .ne. 0) then
            ...
            do j = 1, recv_bl(p+1)%Nparcels
               do i = 1, recv_bl(p+1)%blocksizes(j)
                  a1out(recv_bl(p+1)%displacements(j)+i) = ga_r4_r(offset_v+i)
               enddo
               offset_v = offset_v + recv_bl(p+1)%blocksizes(j)
            enddo
            ...
            if ( present(a2out) ) then
               ...
               do j = 1, recv_bl(p+1)%Nparcels
                  do i = 1, recv_bl(p+1)%blocksizes(j)
                     a2out(recv_bl(p+1)%displacements(j)+i) = ga_r4_r(offset_v+i)
                  enddo
                  offset_v = offset_v + recv_bl(p+1)%blocksizes(j)
               enddo
