...
!   changes:
!             Jun Du, 2013-12-16
!             use model lowest-level values for T2m, q2m, U10m, V10m.
!             RH2m and Td2m are calculated accordingly
!     
...
!$$$  
      ...
      use params_mod, only: pi, dtr, g, d608, rd, p1000, capa
...
! use model lowest-level value at f00
      if(ifhr.eq.0) then
         do j=jsta,jend
          do i=1,im
          qshltr(i,j)=q(i,j,lm)
...
!      write(0,*)' zint=',zint(i,j,l),zint(i,j,l+1)
      ...
      ,jend_2u,MPI_COMM_COMP,icnt,idsp,spval,VarName,VcoordName &
      ,l,impf,jmpf,nframe,pshltr)
! use model lowest-level value at f00 (it will impact T2m etc, so don't use it)
!     if(ifhr.eq.0) then
!       do j=jsta,jend
!        do i=1,im
!         pshltr(i,j)=PMID(i,j,lm)
!        enddo
!       enddo
!     end if
      if(debugprint)print*,'sample ',VarName,' = ',pshltr(im/2,(jsta+jend)/2), &
      ...
      ,l,impf,jmpf,nframe,tshltr)
! use model lowest-level value at f00
! T_model is in T (temp), while T2m is in ths (potential temp)
      if(ifhr.eq.0) then
        do j=jsta,jend
         do i=1,im
          tshltr(i,j)=t(i,j,lm) ! no conversion (closest to wrf_nmm and arw in testing)
!         dummy(I,J)= t(i,j,lm)*(p1000/PMID(i,j,lm))**CAPA ! convert from T to THS (according to Hui-Ya)
!         dummy(I,J)= t(i,j,lm)*(PMID(i,j,lm)/p1000)**CAPA ! convert from THS to T (for testing)
