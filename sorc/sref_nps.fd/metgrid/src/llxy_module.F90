...
module llxy_module
   ...
   type (proj_info), dimension(-MAX_SOURCE_LEVELS:MAX_DOMAINS) :: proj_stack, proj_stack_nestlev
...
!                      domcenlon_loc=proj_stack(current_nest_number)%dom_cen_lon, &
   ...
   subroutine compute_nest_locations()
	...
	print*, 'user_known_x, user_known_y, user_known_lat, user_known_lon: ', &
        ...
        PRINT*, 'call map_set for domain 1 for PROJ_ROTLLB'
        write(0,*)' phi lambda ', phi ,lambda
	...
	print*, 'user_known_x, user_known_y, user_known_lat, user_known_lon: ', temp_known_x, temp_known_y, temp_known_lat, temp_known_lon
            ...
            ixdim_proj = ixdim(1)*(3**nest_level)-(3**nest_level-1)
            jydim_proj = jydim(1)*(3**nest_level)-(3**nest_level-1)

            rmin=5*temp_dxkm

            do jj =  1, jydim_proj
              do ii =  1, ixdim_proj
                call ijll_rotlatlon_noproj(real(ii), real(jj), ixdim_proj, jydim_proj, &
                                   temp_dxkm, temp_dykm, known_lat, known_lon, &
                                   tst_lat, tst_lon )

                val=( ( tst_lat-dom_cen_lat(i) )**2. + &
                      ( tst_lon-dom_cen_lon(i) )**2. ) ** 0.5
                if (val .lt. rmin .and. mod((II-1),3) .eq. 0 .and. mod((JJ-1),3) .eq. 0) then
                  rmin=val
                  print*, ' II, JJ, tst_lat, tst_lon: ',  II, JJ, tst_lat, tst_lon
                  cen_lat=tst_lat
                  cen_lon=tst_lon
                endif
              enddo
