...
module smooth_module
   ...
   subroutine smth_desmth_special(array, start_dom_x, end_dom_x, start_dom_y, end_dom_y, &
      ...
      ! Remove artificially negative and non-zero values
      do iy=start_y,end_y
         do ix=start_x,end_x
            do iz=start_z,end_z
               if (array(ix,iy,iz) < 0. .and. orig_array(ix,iy,iz) >= 0.) then
                  array(ix,iy,iz) = orig_array(ix,iy,iz)
               else if (array(ix,iy,iz) .ne. 0. .and. orig_array(ix,iy,iz) .eq. 0.) then
	          if (abs(array(ix,iy,iz)) .ge. 90.) then
	          print*, 'reset to zero topography at : ', ix,iy, 'from: ', array(ix,iy,iz)
	          endif
                  array(ix,iy,iz) = orig_array(ix,iy,iz)
               end if
            end do
