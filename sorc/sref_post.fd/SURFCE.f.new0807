...
!     SET CELCIUS TO KELVIN AND SECOND TO HOUR CONVERSION.
      real,PARAMETER :: C2K = 273.15, TDmax=C2K+28.0
...
!          EGRID1(I,J)=DWPT
	      ...
	      EVP(I,J)=EVP(I,J)*D001
            ...
            DO J=JSTA,JEND
            DO I=1,IM
! Solution 1 (Brad Ferrier): put a cap if 2m Td exceeds TDmax
              EGRID1(I,J)=MIN(EGRID1(I,J), TDmax)
! Solution 2 (Jun Du): recalculate 2m Td using lowest layer q if it exceeds TDmax
!             if(EGRID1(I,J).gt.TDmax) then
!          print*,'Note: 2m Td exceeds 82F, need to recalculate ...'
!          print*,'I,J, original 2mTd', I,J,EGRID1(I,J)
!          qv = max(1.E-5,(q(i,j,lm)/(1.-q(i,j,lm))))
!          e=PSHLTR(I,J)/100.*qv/(0.62197+qv)
!          EGRID1(I,J) = (243.5*ALOG(E)-440.8)/(19.48-ALOG(E))+273.15
!          print*,'I,J, new 2mTd', I,J,EGRID1(I,J)
!             endif
            ENDDO
            ENDDO
