module storage_module
  ...
  type node2
     ...
     real, pointer, dimension(:,:) :: data2d => NULL()
     type(mapinfo) :: data_map
     type(node2), pointer :: next => NULL()
  end type node2

  type node1
     integer :: id
     type(node1), pointer :: next => NULL()
     type(node2), pointer :: first => NULL()
  end type node1
  ...
  subroutine clear_storage
    ...
    SEARCH : do
       ...
       N2: do 
          ...
          if (associated(hold%next)) then
             deallocate(hold%next)
             nullify(hold%next)
          end if
       enddo N2
       deallocate(holdnn%next)
       nullify(holdnn%next)
