...
C-----------------------------------------------------------------------
                             P A R A M E T E R
     & (NSTAT=1642,LCL1ML=15,LCL1SL=52
     &, D608=0.608)
C-----------------------------------------------------------------------
C
...
C	new stuff
      ...
      character(len=98) :: fileName_prev
...
!         IDATE(1)=iyear
!         IDATE(5)=ihrst
! set NTSD value at forecast hour = 00
          NTSD=1
          NCNVC=4
...
!	write(6,*) 'N, hgt(N) before grav mult: ', N, IDSTN(N), FIS(N)
        FIS(N)=FIS(N)*g
      enddo
...
C  also set 10m wind components to lowest layer values
         DO N=1,NUMSTA
           TLMIN(N)=T(N,LM)
           TLMAX(N)=T(N,LM)
           U10(N)=U(N,LM)
...
C
C SEVERAL VARIABLES ARE UNDEFINED AT F00 AND HAVE
C  WORTHLESS VALUES; TRYING HERE TO ASSIGN A REASONABLE
C  VALUE FOR EACH
      IF(ITAG .eq. 0) THEN
        PRODAT(NWORD15+30) = THS(N)
        PRODAT(NWORD15+31) = Q(N,LM)
        PRODAT(NWORD15+32) = T(N,LM)
        PRODAT(NWORD15+33) = Q(N,LM) 
      ELSE
...
!      NREC=(IFHR/INCR)*NUMSTA+N
      NREC=N
       write(6,*) 'NREC,IHRST,IDAT,NLEN: ', 
     &NREC,IHRST,IDAT,IFCST,NLEN
...
!      call mpi_finalize(mpi_comm_world, ierr)
	...
	write(6,*) 'end of routine PROF_NMMB'
