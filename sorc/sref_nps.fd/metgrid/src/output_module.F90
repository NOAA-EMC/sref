...
module output_module
   ...
   subroutine output_init(nest_number, title, datestr, grid_type, dynopt, &
      ...
      output_fname = ' '
      if (grid_type == 'C') then
         output_fname = trim(opt_output_from_geogrid_path)//'geo_em.d  .dio'
         ...
         if (nest_number == 1) then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmm.d  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
         else
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmm_nest.l  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+15:i+16),'(i2.2)') nest_number-1
         end if
      else if (grid_type == 'B') then
         if (nest_number == 1) then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmb.d  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
         else
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmb_nest.l  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+15:i+16),'(i2.2)') nest_number-1
         end if
      else if (grid_type == 'A') then
         if (nest_number == 1) then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_sla.d  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
         else
            output_fname = trim(opt_output_from_geogrid_path)//'geo_sla_nest.l  .dio'
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+15:i+16),'(i2.2)') nest_number-1
         end if
      end if
      ...
      output_fname = ' '
      if (grid_type == 'C') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_em.d  .'//trim(datestr)//'.dio'
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+9:i+10),'(i2.2)') nest_number
      else if (grid_type == 'E') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_nmm.d  .'//trim(datestr)//'.dio'
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      else if (grid_type == 'B') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_nmb.d  .'//trim(datestr)//'.dio'
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      else if (grid_type == 'A') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_sla.d  .'//trim(datestr)//'.dio'
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      end if
   ...
   subroutine init_output_fields(grid_type, &
      ...
      if (grid_type == 'E' .or. grid_type == 'B') NUM_AUTOMATIC_FIELDS = 7
...
!      for staggering if it is alright to pass true domain dimensions to write_field.
               ...
               istatus = 0
	if (trim(fields(i)%fieldname) .eq. 'STC_WPS') then
	print*, 'maxval STC_WPS: ', maxval(real_dom_array(:,:,1)),  maxval(real_dom_array(:,:,2)),  maxval(real_dom_array(:,:,3)),  maxval(real_dom_array(:,:,4))
	endif


	print*, 'trim(fields(i)%fieldname), dim sizes: ', trim(fields(i)%fieldname), size(real_dom_array,dim=1), size(real_dom_array,dim=2), size(real_dom_array,dim=3)
               call dio_write(dfile,trim(fields(i)%fieldname), real_dom_array, iret=istatus)
   ...
   subroutine ext_put_dom_ti_integer_scalar(var_name, var_value)
      ...
      call dio_write(dfile,trim(var_name),var_value,iret=istatus)
   ...
   subroutine ext_put_dom_ti_integer_vector(var_name, var_value, n)
      ...
      call dio_write(dfile,trim(var_name),var_value(1:n),iret=istatus)
      call mprintf((istatus /= 0),ERROR,'Error in writing domain time-independent attribute')
