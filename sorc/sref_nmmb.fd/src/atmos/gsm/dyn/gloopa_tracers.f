...
!     character*20 hpmlab
     ...
     &                        sigmp_a(:,:,:)
      real    ,allocatable :: lamdp_a(:,:,:),phidp_a(:,:,:),
     &                        sigdp_a(:,:,:)
...
!    &            lam_all_ar(:,:,:), phi_all_ar(:,:,:),
     ...
     &,           ntr,s1,s2,s3
!
      real(kind=kind_evod) deltim, scale_ibm, reall
...
!
      ...
      save  lammp_a,phimp_a,sigmp_a,lamdp_a,phidp_a,sigdp_a,
     & z_m,sd_m,unlm,vnlm,tnlm
      save  etamid,etaint,cosf1,rcos1,ug_m,vg_m,ww_m
...
111     format('in gloopa k=',i3,2x,' pkref=',e10.3,2x,'tkref=',e10.3,
     . ' tb_k=',e12.4,2x,'ak=',e10.3,2x,'bk=',e10.3)
        ...
        allocate ( lamdp_a(lonf,levs,lats_dim_a) )
        allocate ( phidp_a(lonf,levs,lats_dim_a) )
        allocate ( sigdp_a(lonf,levs,lats_dim_a) )
...
c***************************************************
      ...
      allocate (  ug_h2(lon_dim_h,levs  ,lats_dim_h) )
      allocate (  vg_h2(lon_dim_h,levs  ,lats_dim_h) )
      allocate (  ww_h2(lon_dim_h,levs  ,lats_dim_h) )
...
!sk10022012
     ...
     &        dpdt_d3_h1(1,1,1),dpdt_d3_h2(1,1,1),dpdt_d3_a(1,1,1) ,
     &        zdp,
...
!my12272012 begin 
      if (settls_dep3ds .and. settls_dep3dg) then
      call  set_halos(ug_h2,
...
!original non-SETTLS trajectory
      call  set_halos(ug_h,
     &               lats_nodes_h,global_lats_h,
     &               lonsperlat,
     &               lon_dim_h,xhalo,yhalo,
...
!my12282012 begin 
      if (settls_dep3ds .and. settls_dep3dg) then
        call sltini_n_uvw(n_ggp,lon_dim_h,ug_h2,vg_h2,ww_h2,yhalo,
     &                     lonsperlat(latg))                     
        else  
!original non-SETTLS trajectory
...
cmy redundant for ww_h - for testing fix later
     ...
     &                     lonsperlat(latg))                     
        ...
        call sltini_nh_scalar(n_ggp,lon_dim_h,dpdt_d3_h1(1,1,1),yhalo,
     &                     lonsperlat(latg))
        call sltini_nh_scalar(n_ggp,lon_dim_h,dpdt_d3_h2(1,1,1),yhalo,
     &                     lonsperlat(latg))
...
!my12282012 begin 
      if (settls_dep3ds .and. settls_dep3dg) then
        call sltini_s_uvw(s_ggp,lon_dim_h,ug_h2,vg_h2,ww_h2,yhalo,
     &                     lonsperlat(1))                     
        else
!original non-SETTLS trajectory
...
cmy redundant for ww_h - for testing fix later
     ...
     &                     lonsperlat(1))                     
        call sltini_sh_scalar(s_ggp,lon_dim_h,dpdt_d3_h1(1,1,1),yhalo,
     &                     lonsperlat(1))                     
        call sltini_sh_scalar(s_ggp,lon_dim_h,dpdt_d3_h2(1,1,1),yhalo,
     &                     lonsperlat(1))                     
        do ntr=1,ntrac
...
!         endif 
      ...
      call mpi_bcast(spdmax,levs,mpi_r_mpi_r,
     x               0,mc_comp,ierr)
      deallocate ( spdlat )
