     ...
     &  uu_gr_h_3, vv_gr_h_3, ww_gr_h_3, 
     &  ug_m_2, vg_m_2, ww_m_2,
     &  lammp_a, phimp_a, sigmp_a, lamdp_a, phidp_a, sigdp_a, me,
     &  rgt_h, rgt_a, ud3_h1, ud3_h2, ud3_a, vd3_h1, vd3_h2, vd3_a, 
     &  td3_h1, td3_h2, td3_a,
     &  dpdt_d3_h1, dpdt_d3_h2, dpdt_d3_a, zdp_grid,
...
!     ------------------------------------------------------------------
      ...
      real   uu_gr_h_3(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      real   vv_gr_h_3(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      real   ww_gr_h_3(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      real   ug_m_2(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      real   vg_m_2(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      real   ww_m_2(lon_dim_h*levs ,s_lat_in_pe:n_lat_in_pe)
      ...
      real     dpdt_d3_h1(lon_dim_h,levs ,s_lat_in_pe:n_lat_in_pe)
      real     dpdt_d3_h2(lon_dim_h,levs ,s_lat_in_pe:n_lat_in_pe)

      real  lammp_a(lonf,levs,lats_node_a)
      real  phimp_a(lonf,levs,lats_node_a)
      real  sigmp_a(lonf,levs,lats_node_a)
      real  lamdp_a(lonf,levs,lats_node_a)
      real  phidp_a(lonf,levs,lats_node_a)
      real  sigdp_a(lonf,levs,lats_node_a)
     ...
     &    dhl(i_1:i_2,plev,4),dhr(i_1:i_2,plev,4)
      ...
      logical her_x,her_y,her_z,lprnt
      real    tem
!    -------------------------------------------------------------------
...
!     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
     ...
     &            - global_lats_a(ipt_lats_node_a+lats_node_a-lan_loc)
          ...
          enddo
!sk10052012 my12272012
          if (settls_dep3ds.or.settls_dep3dg) then
          do k=1,levs
            do i=1,lonsperlat(lat_loc)
               lamdp_a(i,k,lan_loc) = lammp_a(i,k,lan_loc)
               phidp_a(i,k,lan_loc) = phimp_a(i,k,lan_loc)
               sigdp_a(i,k,lan_loc) = sigmp_a(i,k,lan_loc)
            enddo
          enddo
          do i=1,lonsperlat(lat_loc)
            sigdp_a(i,plev,lan_loc) = sigmp_a(i,plev,lan_loc)
          enddo
          endif
...
!sk10052012
     ...
     &                 jcen, ztodt, iter, ra,
     &                 uu_gr_h_3(1,s_lat_in_pe),
     &                 vv_gr_h_3(1,s_lat_in_pe),
     &                 ww_gr_h_3(1,s_lat_in_pe),
     &                 lam, phi, dphi, etamid, detam, kdpmpf,
     &                 lamdp_a(1,1,lan), phidp_a(1,1,lan),
     &                 sigdp_a(1,1,lan),
...
!sela      print*,' entering herxin_h'
     ...
     &                 x2,x3,term1x,term2x,term3x,term4x,fintx,qmx_trac)
           endif
           if (her_y) then
     ...
     &                   kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &           term1z,term2z,term3z,term4z,rgt_a(1,1,lan,nt),qmz_trac)
           endif
...
!-----------------------------------------------------------------------
     ...
     &                    term1z,term2z,term3z,term4z,ud3_d)
             ...
             call lagzin_h_m(i_1,i_2,
     &                    kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &                  term1z,term2z,term3z,term4z,ud3_d,qmz_u)
          endif
c
         if (her_x) then
           call herxin_h(i_1,i_2,s_lat_in_pe,n_lat_in_pe,idp,jdp,kdp,
     ...
     &             x2,x3,term1x,term2x,term3x,term4x,fintx,qmx_v)
         endif
     ...
     &                    term1z,term2z,term3z,term4z,vd3_d)
             ...
             call lagzin_h_m(i_1,i_2,
     &                    kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &                    term1z,term2z,term3z,term4z,vd3_d,qmz_v)
          endif
...
!-----------------------------------------------------------------------
     ...
     &              x2,x3,term1x,term2x,term3x,term4x,fintx,qmx_t)
     ...
     &                   term1z,term2z,term3z,term4z,td3_a(1,1,lan))
           ...
           call lagzin_h_m(i_1,i_2,
     &                   kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &          term1z,term2z,term3z,term4z,td3_a(1,1,lan),qmz_t)
         endif
       endif  !  if (her_x.and.her_y.and.her_z) then
!-----------------------------------------------------------------------
...
!sk10302012 (ia) tricubic interpolation for dpdt_d3_h1
          call lagxin_h_m(i_1,i_2,s_lat_in_pe,n_lat_in_pe,
     &               kdim,dpdt_d3_h1,idp,jdp,kdp,kkdp,
     &        x2,x3,term1x,term2x,term3x,term4x,fintx,quamon)
          call lagyin_h_m(i_1,i_2,fintx,finty,yb,yt,term1y,term2y,
     &               term3y,term4y,quamon)
          call lagzin_h_m(i_1,i_2,
     &               kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &       term1z,term2z,term3z,term4z,dpdt_d3_a(1,1,lan),quamon)
        else 
...
!sk10302012 (ii) bicubic interpolation for dpdt_d3_h2
     ...
     &               term1y,term2y,term3y,term4y,
     &              dpdt_d3_a(1,1,lan))
!use linear interp in x and y for nonlinear term and add to advected
!lnps 
         call lin_xy_in_h(i_1,i_2,s_lat_in_pe,n_lat_in_pe,
     &      kdim,dpdt_d3_h2,idp,jdp,x2,x3,yb,yt,dpdt_d3_a(1,1,lan))
      else
...
!
            if (dist(i,k) > distmx) then
              ...
              phipi2 = asin((sqrt((slam2-1.)/(slam2-1./cphic**2)))*fac)
...
!
     ...
     &     vb (plond,plev ,s_lat_in_pe:n_lat_in_pe),
     &     wb (plond,plev ,s_lat_in_pe:n_lat_in_pe),
     &     un (plond,plev ,s_lat_in_pe:n_lat_in_pe),
     &     vn (plond,plev ,s_lat_in_pe:n_lat_in_pe),
     &     wn (plond,plev ,s_lat_in_pe:n_lat_in_pe),
     &     uxl(i_1:i_2,plev ,2,platd),
     &     uxr(i_1:i_2,plev ,2,platd)
      real detam(plev)
      real etamid(plev)           ! eta at levels
      real dt,ra,lam(plond,platd),phib(platd),dphib(platd),
     &     lamdp_a(plon,plev),phidp_a(plon,plev),sigdp_a(plon,plev),
     &     lamdp(i_1:i_2,plev),phidp(i_1:i_2,plev),sigdp(i_1:i_2,plev)
     ...
     &     ys(i_1:i_2,plev),yn(i_1:i_2,plev)
      integer iter,i,k,twofld
      real udp(i_1:i_2,plev),vdp(i_1:i_2,plev),wdp(i_1:i_2,plev),
     &     una(i_1:i_2,plev),vna(i_1:i_2,plev),wna(i_1:i_2,plev),
     &     upr(i_1:i_2,plev),vpr(i_1:i_2,plev),
     &     phicen,cphic,sphic,
     &     lampr(i_1:i_2,plev),phipr(i_1:i_2,plev),
     &     sigpr(i_1:i_2,plev),hfdt
     ...
     &     slampr(i_1:i_2,plev),dlamx(i_1:i_2,plev),coeff,distmx,
     &       dist(i_1:i_2,plev)
      real cdlam,clamp,cphidp,cphip,sdlam,slamp,sphidp,sphip
      real rdel
      integer kk,ii,jj
      integer bisection
...
!
      ...
      if ((iter==1) .and. iter_one_no_interp) then
        ...
        do k = 1,plev
          do i=i_1,i_2
            jdp(i,k) = int ( (phidp_a(i,k) - phibs)*dphibr + 1. )

            if( phidp_a(i,k) >= phib(jdp(i,k)+1) )
     &          jdp(i,k) = jdp(i,k) + 1
!
            idp(i,k,1) = 2 + int( lamdp_a(i,k) * rdlam(jdp(i,k)-1) )
            if (redgg_a) then
               idp(i,k,2) = 2 + int( lamdp_a(i,k) * rdlam(jdp(i,k)) )
               idp(i,k,3) = 2 + int( lamdp_a(i,k) * rdlam(jdp(i,k)+1) )
               idp(i,k,4) = 2 + int( lamdp_a(i,k) * rdlam(jdp(i,k)+2) )
            else
               idp(i,k,2) = idp(i,k,1)
               idp(i,k,3) = idp(i,k,1)
               idp(i,k,4) = idp(i,k,1)
...
!
        call xywgts_h(i_1,i_2,
     &              lam,phib,dphib,idp,jdp,lamdp_a,phidp_a,xl,xr,ys,yn)
        call int3dv_h(i_1,i_2,s_lat_in_pe,n_lat_in_pe,
     &                ub,vb,etamid,detam,sigdp_a,idp,jdp,kdp,
     &                xl,xr,ys,yn,udp,vdp,
     &                wb,wdp)
      endif
!
        do k = 1,plev
           do i = i_1,i_2
              una(i,k) = un(i+1,k,jcen)*ra
              vna(i,k) = vn(i+1,k,jcen)*ra
              wna(i,k) = wn(i+1,k,jcen)
           enddo
        enddo
!
        do k = 1,plev
          do i = i_1,i_2
            ...
            clamp  = cos( asin( slamp ) )
            vpr(i,k) = (vdp(i,k)*(cphidp*cphic + sphidp*sphic*cdlam) -
     &                  udp(i,k)*sphic*sdlam)/cphip
            upr(i,k) = (udp(i,k)*cdlam + vdp(i,k)*sphidp*sdlam +
     &                  vpr(i,k)*slamp*sphip)/clamp
 
            lampr(i,k) = -hfdt*( upr(i,k)/cos(phipr(i,k))
     &                          +una(i,k) )
            phipr(i,k) = -hfdt*(vpr(i,k)+vna(i,k))
          enddo
        enddo
...
!
     ...
     &      phidp_a(i,k) = sign( phib(3+plat),phidp_a(i,k) )*fac
            dlamx(i,k)=asin((slampr(i,k)*cphipr/cos(phidp_a(i,k)))*fac)
            dist(i,k) = upr(i,k)*upr(i,k) + vpr(i,k)*vpr(i,k)
!
            if (dist(i,k) > distmx) then
              slam2 = slampr(i,k)*slampr(i,k)
...
!
            lamdp_a(i,k) = lam(i+1,jcen) + dlamx(i,k)
            if( lamdp_a(i,k) >= twopi ) lamdp_a(i,k)=lamdp_a(i,k)-twopi
            if( lamdp_a(i,k) <  0.0 )   lamdp_a(i,k)=lamdp_a(i,k)+twopi
!
            sigpr(i,k) =  -hfdt*(wdp(i,k)+wna(i,k))
            sigdp_a(i,k) = etamid(k) + sigpr(i,k)
            if (sigdp_a(i,k) < etamid(1)) then
                sigdp_a(i,k) = etamid(1)
            endif
            if (sigdp_a(i,k) >= etamid(plev)) then
                sigdp_a(i,k) = etamid(plev) - mprec
            endif
          enddo
        enddo
      enddo settls_loop 
...
!
     ...
     &        ys(i_1:i_2,plev)  , yn(i_1:i_2,plev)
      integer iter,i,k,twofld
      real udp(i_1:i_2,plev),vdp(i_1:i_2,plev),wdp(i_1:i_2,plev)
      real una(i_1:i_2,plev),vna(i_1:i_2,plev),wna(i_1:i_2,plev),
     &     phicen,lampr(i_1:i_2,plev),phipr(i_1:i_2,plev),
     &                     sigpr(i_1:i_2,plev),pi,twopi,hfdt
      real dphibr,phibs, rdel
      integer kk,ii,jj,bisection
...
!
        do k = 1,plev
          do i = i_1,i_2
            ...
            vdp(i,k) = vdp(i,k)*ra
!
            lampr(i,k) =  -hfdt*(udp(i,k)/cos(phidp_a(i,k))+
     &                           una(i,k)/cos(phicen))
            phipr(i,k) =  -hfdt*(vdp(i,k)+vna(i,k))
            sigpr(i,k) =  -hfdt*(wdp(i,k)+wna(i,k))
            lamdp_a(i,k) = lam(i+1,jcen) + lampr(i,k)
            phidp_a(i,k) = phicen        + phipr(i,k)
            sigdp_a(i,k) = etamid(k)     + sigpr(i,k)

            if(lamdp_a(i,k) >= twopi) lamdp_a(i,k)=lamdp_a(i,k)-twopi
            if(lamdp_a(i,k) <    0.0) lamdp_a(i,k)=lamdp_a(i,k)+twopi
!
            if (sigdp_a(i,k) < etamid(1)) sigdp_a(i,k) = etamid(1)
            if (sigdp_a(i,k) >= etamid(plev))
     &                                sigdp_a(i,k) = etamid(plev)-mprec
          enddo
        enddo
      enddo settls_loop
...
!
     ...
     &       - fb1 (ii4  ,k,jj+2) * x3    (i,k,4)
!
          fdp2(i,kk) = f1*term1y(i,k) + f2*term2y(i,k)
!         fdp1(i,kk) = f1*term1y(i,k) + f2*term2y(i,k)
     &               + f3*term3y(i,k) + f4*term4y(i,k)
          if (quamon) then
            call quasim(f2,f3,fdp2(i,kk))
!           call quasim(f2,f3,fdp1(i,kk))
          endif
          fdp1(i,kk) = fdp1(i,kk) + fdp2(i,kk)
        end do
      end do
      return
      end
...
!
     ...
     *  fdp1(plon,plev)
      ...
      real f2,f3
      real x2(i_1:i_2,plev,4),x3(i_1:i_2,plev,4)
      real yb(i_1:i_2,plev),yt(i_1:i_2,plev)
      integer ii2,ii3,jj
!
      do k=1,plev
        kk = plev+1-k
     ...
     &         + fb1 (ii2+1,k,jj  ) * x2(i,k,2)
          f3 = - fb1 (ii3  ,k,jj+1) * x3(i,k,3)
     &         + fb1 (ii3+1,k,jj+1) * x2(i,k,3)
!
...
!sk   quasi-monotone correction of fint 
     ...
     &                                fb1,idp,jdp,kdp,xl,xr,ys,yn,zb,zt,
     &                                fdp1)
      use      pmgrid        , only : plev,plevp,plon,plond,wgt_slg
      implicit none
      integer i_1,i_2,s_lat_in_pe,n_lat_in_pe
      integer idp(i_1:i_2,plev,4), jdp(i_1:i_2,plev),
     &        kdp(i_1:i_2,plev)
...
!     enddo
     ...
     &                  kdim,finty,lbasdz,kdp,hb,ht,dhb,dht,detam,
     &                  term1,term2,term3,term4,fdp,quamon)
!cmy renamed with _m
