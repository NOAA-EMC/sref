...
!
      ...
      use ctlblk_mod, only: jsta, jend, spval, im, jm, ifhr
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      implicit none
!     
!     DECLARE VARIABLES.
      ...
      DO J=JSTA,JEND
        DO I=1,IM
         ...
         ELSE IF(TCLD(I,J).GE.50.) THEN
           CEILING(I,J)=CLDZ(I,J) - FIS(I,J)*GI    !AGL
!          CEILING(I,J)=CLDZ(I,J)                  !ASL
         ELSE
           CEILING(I,J)=20000.0
         END IF
...
! No ceiling at f00, set a constant value (Jun Du, 12/18/13)
!     if(ifhr==00) CEILING=SPVAL
      if(ifhr==00) CEILING=9999
