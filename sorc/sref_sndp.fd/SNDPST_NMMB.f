...
C--------------------------------------------------------------------
      INCLUDE "parm.nmmb"
      PARAMETER (LM=LM1,NPNT=88,NSTAT=1815
...
C----------------------------------------------------------------------
 ...
 33   CONTINUE
      NREC=NREC+1
      write(STDOUT,*) 'will read NREC from LUNIT: ', NREC,LUNIT
c     READ(LUNIT,REC=NREC,IOSTAT=IRR) IHRST,IDATE,IFCST,
c     READ(LUNIT,REC=NREC,ERR=999) IHRST,IDATE,IFCST,
...
c     IF(IRR.NE.0) THEN
c      WRITE(*,*) NREC, '  read error, IRR=',IRR
c      goto 999
c     END IF
...
C   CWTR, RAIN, AND IMXR NOW IN SEPARATE ARRAYS 
 ...
 25     CONTINUE
      WRITE(STDOUT,*) 'P,T,U,V,Q,CWTR,RAIN,IMXR at 1:',
     &P(LMH),T(LMH),U(LMH),V(LMH),Q(LMH),
     &CWTR(LMH),RAIN(LMH),IMXR(LMH)
