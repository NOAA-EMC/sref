...
! Rotated Lat/Lon (code = PROJ_ROTLLB)
!
! REMARKS
! -------
...
!         domcenlat_loc = center latitude of grid, relevant for NMMB nesting
!         domcenlon_loc = center latitude of grid, relevant for NMMB nesting
!         i_parent_start = starting i point for parent domain (relevant for NMMB nesting)
!         j_parent_start = starting j point for parent domain (relevant for NMMB nesting)
!         i_parent_end = ending i point for parent domain (relevant for NMMB nesting)
!         j_parent_end = ending j point for parent domain (relevant for NMMB nesting)
!
!
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   TYPE proj_info
      ...
      INTEGER          :: i_parent_start ! parent domain starting I value
      INTEGER          :: j_parent_start ! parent domain starting J value
      INTEGER          :: i_parent_end   ! parent domain ending I value
      INTEGER          :: j_parent_end   ! parent domain ending J value
      INTEGER          :: top_i_parent_start
      INTEGER          :: top_j_parent_start
      ...
      REAL             :: domcenlat_loc ! Center latitude of NMMB nest
      REAL             :: domcenlon_loc ! Center longitude of NMMB nest
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE map_init(proj)
      ...
      proj%truelat1 = -999.9
      proj%truelat2 = -999.9
      proj%domcenlat_loc = -999.9
      proj%domcenlon_loc = -999.9
      proj%phi      = -999.9
      proj%lambda   = -999.9
      proj%ixdim    = -999
      proj%jydim    = -999
      proj%wbd      = -999
      proj%sbd      = -999
      proj%i_parent_start = 0
      proj%j_parent_start = 0
      proj%i_parent_end   = 0
      proj%j_parent_end   = 0
      proj%stagger  = HH
      proj%nlat     = 0
   ...
   SUBROUTINE map_set(proj_code, proj, lat1, lon1, lat0, lon0, knowni, knownj, dx, latinc, &
                      loninc, stdlon, truelat1, truelat2, domcenlat_loc, domcenlon_loc, nlat, nlon, ixdim, jydim, &
                      wbd, sbd, i_parent_start, j_parent_start, i_parent_end, j_parent_end, stagger, phi, lambda, r_earth)
      ...
      REAL, INTENT(IN), OPTIONAL        :: wbd
      REAL, INTENT(IN), OPTIONAL        :: sbd
      INTEGER, INTENT(IN), OPTIONAL     :: i_parent_start
      INTEGER, INTENT(IN), OPTIONAL     :: j_parent_start
      INTEGER, INTENT(IN), OPTIONAL     :: i_parent_end
      INTEGER, INTENT(IN), OPTIONAL     :: j_parent_end
      ...
      REAL, INTENT(IN), OPTIONAL        :: domcenlat_loc
      REAL, INTENT(IN), OPTIONAL        :: domcenlon_loc
      ...
      ELSE IF ( proj_code == PROJ_GAUSS ) THEN
         IF ( .NOT.PRESENT(nlat) .OR. &
            ...
            call mprintf(.true.,ERROR,'MAP_INIT')
         END IF
      ELSE IF ( proj_code == PROJ_ROTLL .or. proj_code == PROJ_ROTLLB ) THEN
	...
	IF (proj_code == PROJ_ROTLLB) THEN
!	PRINT '(A)', 'LOOKS LIKE WE ARE PROJ_ROTLLB HERE' 
!        PRINT '(A,f8.4)', 'HAVE lat1 of : ', lat1
        ENDIF
      ...
      IF ( PRESENT(domcenlat_loc) ) proj%domcenlat_loc= domcenlat_loc
      IF ( PRESENT(domcenlon_loc) ) proj%domcenlon_loc= domcenlon_loc
      ...
      IF ( PRESENT(wbd) )    proj%wbd    = wbd
      IF ( PRESENT(sbd) )    proj%sbd    = sbd
      IF ( PRESENT(i_parent_start) )    proj%i_parent_start = i_parent_start
      IF ( PRESENT(j_parent_start) )    proj%j_parent_start = j_parent_start
      IF ( PRESENT(i_parent_end) )    proj%i_parent_end = i_parent_end
      IF ( PRESENT(j_parent_end) )    proj%j_parent_end = j_parent_end
...
!      PRINT*, 'SETTING proj%init to TRUE'
      proj%init = .TRUE.
   ...
   SUBROUTINE latlon_to_ij(proj, lat, lon, i, j)
      ...
      IF (.NOT.proj%init) THEN
         PRINT '(A)', 'You have not called map_set for this projection(A)!'
      ...
      SELECT CASE(proj%code)
         ...
         CASE(PROJ_ROTLLB)
            CALL llij_rotlatlon_bgrid(lat,lon,proj,i,j)
   ...
   SUBROUTINE ij_to_latlon(proj, i, j, lat, lon )
      ...
      IF (.NOT.proj%init) THEN
         PRINT '(A)', 'You have not called map_set for this projection(B)!'
         PRINT*, 'proj%code for this unset projection is: ', proj%code
      ...
      SELECT CASE (proj%code)
         ...
         CASE (PROJ_ROTLLB)
            CALL ijll_rotlatlon_bgrid(i, j, proj, lat, lon)
...
!!! Added next line as a Kludge - not yet understood why needed
   ...
   SUBROUTINE llij_rotlatlon_bgrid(lat, lon, proj, i, j)
...
!-------------------------------------------------------------------------------
      ...
      real(kind=DOUBLE), parameter :: dtr=pi/180.0
!
      real(kind=DOUBLE) :: tlmd, tphd, tph0, ctph0, stph0, relm, srlm, crlm
      real(kind=DOUBLE) :: aph, sph, cph, cc, anum, denom
!-------------------------------------------------------------------------------
!
      if (proj%lon1==0.0.and.proj%lat1==0.0) then
      tlmd=lat
      tphd=lon
      else
...
!
      IF (proj%stagger == VV) THEN
         tlmd=tlmd+0.5*proj%loninc
         tphd=tphd+0.5*proj%latinc
      END IF

      i = (tlmd - proj%wbd)/proj%loninc + 1
      j = (tphd - proj%sbd)/proj%latinc + 1

   END SUBROUTINE llij_rotlatlon_bgrid
...
!!!         ncol = 2*ih-1+MOD(jh+1,2)
   ...
   END SUBROUTINE ijll_rotlatlon
     ...
     SUBROUTINE ijll_rotlatlon_bgrid(i, j, proj, lat, lon)
!-----------------------------------------------------------------------
!
!***  GIVEN THE (I,J) OF MASS POINTS ON THE ARAKAWA B-GRID,
!***  COMPUTE THE GEOGRAPHIC LATITUDES AND LONGITUDES.
!
!-----------------------------------------------------------------------
      IMPLICIT NONE
!-----------------------------------------------------------------------
...
!  +                                                                           +
!  +     tlmd  - transformed longitude, degs                                   +
!  +     tphd  - transformed latitude, degs                                    +
...
!-------------------------------------------------------------------------------
      real(kind=DOUBLE), parameter :: pi=3.141592654
      real(kind=DOUBLE), parameter :: dtr=pi/180.0
      real(kind=DOUBLE) :: tlmd, tphd, tph0, ctph0, stph0, tlmr, tphr, stlm, ctlm, stph, ctph
      real(kind=DOUBLE) :: sph, aph, anum, denom, relm
!-------------------------------------------------------------------------------
!
      if (proj%stagger == HH) then
        tlmd = proj%wbd  + (i-1.)*proj%loninc
        tphd = proj%sbd  + (j-1.)*proj%latinc
      else
        tlmd = proj%wbd  + (i-1.)*proj%loninc + 0.5*proj%loninc
        tphd = proj%sbd  + (j-1.)*proj%latinc + 0.5*proj%latinc
      end if

! fix for imprecise floating point arithmetics
      if (abs(tlmd-180.0)<0.01*proj%loninc .and. abs(tlmd-180.0)/=0.0) then
        write(0,*)' wbd, i, i-1, dx, tlmd ', proj%wbd, i, i-1,proj%loninc, tlmd
        write(0,*)' possible floating point imprecision. tlmd set to 180.0'
        tlmd=180.0
      end if
      if (abs(tphd- 90.0)<0.01*proj%latinc .and. abs(tphd- 90.0)/=0.0) then
        write(0,*)' sbd, j, i-1, dy, tphd  ', proj%sbd, j, j-1,proj%latinc, tphd
        write(0,*)' possible floating point imprecision. tphd set to  90.0'
        tphd= 90.0
      end if
...
!
      ...
      lat=aph/dtr
      anum=ctph*stlm
!dule      denom=(ctlm*ctph-stph0*sph)/ctph0
      if (abs(ctph0) > 1.0e-4) then
         denom=(ctlm*ctph-stph0*sph)/ctph0
      else
!dule         denom=(ctlm*ctph-stph0*(ctph0*stph+stph0*ctph*ctlm))/ctph0
         denom=(-ctph0*stph)/ctph0
      end if

      relm=atan2(anum,denom)
      lon=relm/dtr+proj%lon1
!
      if (lon > 180.0) then
         lon=lon-360.0
      end if
...
!
      return
      END SUBROUTINE ijll_rotlatlon_bgrid

! ------------------------------------------------------------------------------------------
! ------------------------------------------------------------------------------------------
! ------------------------------------------------------------------------------------------

     SUBROUTINE ijll_rotlatlon_noproj(i, j, ixdim, jydim, dlmd, dphd, tph0d, tlm0d, lat, lon)
!-----------------------------------------------------------------------
!
!***  GIVEN THE (I,J) OF MASS POINTS ON THE ARAKAWA B-GRID,
!***  COMPUTE THE GEOGRAPHIC LATITUDES AND LONGITUDES.
...
!-----------------------------------------------------------------------
      INTEGER, PARAMETER :: DOUBLE=SELECTED_REAL_KIND(P=13,R=200)
!-----------------------------------------------------------------------
      LOGICAL,PARAMETER :: GLOBAL=.FALSE.                                  ! Global (.TRUE.) or regional (.FALSE.) domain
!-----------------------------------------------------------------------
!
      INTEGER :: IADD,IEND,JEND, ih,jh, IXDIM, JYDIM

      REAL,INTENT(IN)    :: I,J, tph0d, tlm0d, dlmd, dphd
      REAL,INTENT(OUT)    :: lat,lon
!
      REAL(KIND=DOUBLE) :: ARG1,ARG2,COL_MID,D2R,FCTR,GLATR &
...
!-----------------------------------------------------------------------
      ...
      tph0 = TPH0D*d2r
!       switched so negative west longitude can be entered in namelist.wps
!      tlm0 = proj%lon1*d2r
      tlm0 = -TLM0D*d2r
!       write(6,*) 'tph0, tlm0: ', tph0, tlm0
!
      ROW_MID=(JYDIM+ONE)*HALF
      COL_MID=(IXDIM+ONE)*HALF
!
        IADD=0
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
!purely HH       IF (proj%stagger == HH) THEN
        TLATD=(JH-ROW_MID+IADD)*DPHD
        TLOND=(IH-COL_MID)*DLMD
!       ELSE
!        TLATD=(JH-ROW_MID+IADD)*DPHD+DPHD_HALF
!        TLOND=(IH-COL_MID)*DLMD+DLMD_HALF
!       ENDIF

!
!        if (mod(IH,15) .eq. 0 .and. (JH .eq. JEND .or. JH .eq. JEND-1) ) then
!        WRITE(6,50)IH,JH,TLATD,TLOND
!        endif
...
!
        ...
        IF(TLOND>0.)FCTR=-1.
!
!        lon=TLM0D+FCTR*ACOS(ARG2)*R2D
        lon=-TLM0D+FCTR*ACOS(ARG2)*R2D
!
!        if (mod(IH,10) .eq. 0 .and. mod(JH,10) .eq. 0) then
...
!
      ...
      END SUBROUTINE ijll_rotlatlon_noproj
