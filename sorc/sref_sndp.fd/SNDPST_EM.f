...
C
       PTOP=25.0*100.0
       READ(5,MODTOP,ERR=12321)  !read from standard io (screen)
12321  CONTINUE
       PTOP=ETOP*100.0
       WRITE(STDOUT,*) 'PTOP=',PTOP
C
...
C
       MONOL=.TRUE.
       BRKOUT=.FALSE.
       READ(11,OPTION,ERR=12322) !read from file eta_sndp.parm.mono
12322  CONTINUE
       WRITE(STDOUT,*) 'MONOL,BRKOUT=',MONOL,BRKOUT
C
cZHou        IFCSTL=-99
cZhou        JHR=0
C----------------------------------------------------------------------
...
cZHOU      NREC=NREC+1      
     ...
     &   ISTAT,CISTAT,(FPACK(N),N=1,9),(FPACK(N),N=10,FPACK(7))
      WRITE(STDOUT,*) 'IHRST,IDATE,IFCST,ISTAT,CISTAT=',
     &IHRST,IDATE,IFCST,ISTAT,CISTAT,
     &' FPACK(1-7)=',(FPACK(K),K=1,7)
...
C   IF THE CLOUD WATER IS NEGATIVE, IT IS ICE
         ...
         ENDIF
 25     CONTINUE
      WRITE(STDOUT,*) 'P,T,U,V,Q,CWTR,IMXR at 1:',
     &P(LMH),T(LMH),U(LMH),V(LMH),Q(LMH),
     &CWTR(LMH),IMXR(LMH)
...
c     WRITE(STDOUT,*) 'single layer variables:',
c    &(FPACK(i),i=LCL1ML*LMH+10,LCL1ML*LMH+LCL1SL+10)
