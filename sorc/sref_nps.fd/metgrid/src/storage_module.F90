   ...
   subroutine storage_delete_field(delete_me)
      ...
      do while ( associated(data_cursor) )
         if (secondary_cmp(delete_me, data_cursor%fg_data) == EQUAL) then
            ...
            if (associated(data_cursor%fg_data%valid_mask)) then
               call bitarray_destroy(data_cursor%fg_data%valid_mask)
               deallocate(data_cursor%fg_data%valid_mask)
               nullify(data_cursor%fg_data%valid_mask)
            end if
            if (associated(data_cursor%fg_data%modified_mask)) then
               call bitarray_destroy(data_cursor%fg_data%modified_mask)
               deallocate(data_cursor%fg_data%modified_mask)
