...
contains
    ...
    if (ierr .ne. 0) then
        ...
        call wrf_error_fatal3("module_io_int_read.F90", 134, message)
        return
    end if
    dst = ntohl(tmp)
...
!    call wrf_debug(2, message)
    ...
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 173, &
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 188, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 233, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 248, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 295, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 311, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 350, message)
        return
    end if
...
!    call wrf_debug(2, message)
    ...
    allocate(tmp(its:ite), stat=ierr)
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 389, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 404, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    allocate(tmp(its:ite, jts:jte), stat=ierr)
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 449, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 464, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    allocate(tmp(its:ite, jts:jte, kts:kte), stat=ierr)
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 511, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 527, message)
        return
    end if
...
!     call wrf_debug(2, message)
    ...
    if (ierr .ne. 0) then
        call wrf_error_fatal3("module_io_int_read.F90", 568, &
             'Unable to allocate a temporary array')
        return
    end if
    tmp = 0
    ...
    if (ierr .ne. 0) then
        write(message,*) 'MPI IO: Unable to read ', varname
        call wrf_error_fatal3("module_io_int_read.F90", 578, message)
        return
    end if
